...
@section('content')
    ...
    <div class="layout-login__form bg-white" data-perfect-scrollbar>
        ...
        <h4 class="m-0 text-center">Login </h4><br>
       

        <form action="{{ route('admin.login') }}" novalidate method="POST">
		@csrf
            <div class="form-group">
                <label class="text-label" for="email_2">{{ __('E-Mail Address') }}</label>
                <div class="input-group input-group-merge">
                    <input id="email_2" type="email" value="{{ old('email') }}" required autocomplete="email" autofocus name="email" class="form-control form-control-prepended  @error('email') is-invalid @enderror" placeholder="john@doe.com">
					@error('email')
            ...
            <div class="form-group">
                ...
                <div class="input-group input-group-merge">
                    <input id="password_2" type="password" required autocomplete="current-password" class="form-control form-control-prepended @error('password') is-invalid @enderror" name="password"  placeholder="Enter your password">
					@error('password')
                                    <span class="invalid-feedback" role="alert">
                                        <strong>{{ $message }}</strong>
                                    </span>
            ...
            <div class="form-group text-center">
				...
				@if (Route::has('admin.password.request'))			
                   <a class="btn btn-link" href="{{ route('admin.password.request') }}">Forgot password?</a>
                @endif
			<br>
               @if (Route::has('admin.register'))
								 Don't have an account? 
                                    <a class="text-body text-underline" href="{{ route('register') }}">{{ __('Sign up!') }}</a>
